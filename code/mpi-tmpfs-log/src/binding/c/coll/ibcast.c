...
/*@
   MPI_Ibcast_c - Broadcasts a message from the process with rank "root" to all other processes of the communicator in a nonblocking way

Input Parameters:
+ count - number of entries in buffer (non-negative integer) (non-negative integer)
. datatype - data type of buffer (handle) (handle)
. root - rank of broadcast root (integer) (integer)
- comm - communicator (handle) (handle)

Input/Output Parameters:
. buffer - starting address of buffer (choice) (choice)
