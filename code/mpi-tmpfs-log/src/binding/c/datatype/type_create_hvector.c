...
/*@
   MPI_Type_create_hvector_c - Create a datatype with a constant stride given in bytes

Input Parameters:
+ count - number of blocks (non-negative integer) (non-negative integer)
. blocklength - number of elements in each block (non-negative integer) (non-negative integer)
. stride - number of bytes between start of each block (integer) (integer)
