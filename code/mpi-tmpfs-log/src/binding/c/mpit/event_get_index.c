...
{
    return internal_T_event_get_index(name, event_index);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_T_event_get_index(const char *name, int *event_index)
{
    QMPI_Context context;
    QMPI_T_event_get_index_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_T_event_get_index(context, 0, name, event_index);

    fn_ptr = (QMPI_T_event_get_index_t *) MPIR_QMPI_first_fn_ptrs[MPI_T_EVENT_GET_INDEX_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_T_EVENT_GET_INDEX_T], name,
            event_index);
}
#else /* ENABLE_QMPI */
int MPI_T_event_get_index(const char *name, int *event_index)
