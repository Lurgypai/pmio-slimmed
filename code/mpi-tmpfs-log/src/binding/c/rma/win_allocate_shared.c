...
/*@
   MPI_Win_allocate_shared_c - Create an MPI Window object for one-sided communication and shared memory access, and allocate memory at each process

Input Parameters:
+ size - size of local window in bytes (non-negative integer) (non-negative integer)
...
Output Parameters:
+ baseptr - address of local allocated window segment (choice) (choice)
- win - window object returned by the call (handle) (handle)
