...
// #define LOG_BUF_SIZE 1024*1024*100ULL
// #define DATA_BUF_SIZE 1024*1024*100ULL
...
#define DISK_STRIPE_SIZE 16384*1024ULL
...
struct log_item_s
{
   ...
   uint64_t ptr : 48;             // Address in persistent memory
...
typedef struct ADIOI_FileD {
    ...
    ADIO_Offset pm_cur_pos;      /* current buffer position*/
    void *log_buf_addr;          /* pesistent log buffer, only used in writes */
    void *data_buf_addr;	 /* persistent data buffer, only used in writes*/
    log_t *log;                  /* log file head pointer*/
    lchunk_t *cur_lchunk[N_IO_SERVER];          /* current lchunk*/ 
    uint64_t num_of_log_items;   /* number of items in the log*/
    uint64_t num_of_chunks;      /* number of chunks in the log*/
    uint64_t log_buf_used;	 /* log space used*/
    uint64_t data_buf_used;     /* data space used */
    uint64_t num_of_data_items;  /* number of items in data buffers*/
    uint64_t version;		 /* version of data in the log*/
