...
/* create and initialize info object */
    ...
    // open a file
    const uint64_t buff_size = fd->hints->cb_buffer_size;
    const char* log_fname = "/tmp/log_buffers/log_buffer";
    const char* data_fname = "/tmp/data_buffers/data_buffer";

    char buffer[256];
    sprintf(buffer, "%s_%d", log_fname, rank);
    int file = open(buffer, O_RDWR | O_CREAT | O_TRUNC, 0666);
    ftruncate(file, 600000);
    fd->log_buf_addr = mmap(NULL, buff_size, PROT_READ | PROT_WRITE, MAP_SHARED, file, 0);
    close(file);

    
    sprintf(buffer, "%s_%d", data_fname, rank);
    file = open(buffer, O_RDWR | O_CREAT | O_TRUNC, 0666);
    ftruncate(file, buff_size);
    fd->data_buf_addr = mmap(NULL, buff_size, PROT_READ | PROT_WRITE, MAP_SHARED, file, 0);
    close(file);
