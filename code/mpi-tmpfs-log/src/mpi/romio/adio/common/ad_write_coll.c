...
static void ADIOI_Exch_and_write_pm(ADIO_File fd, void *buf, MPI_Datatype
                                 datatype, int nprocs, int myrank,
                                 ADIOI_Access
                                 * others_req, ADIO_Offset * offset_list,
                                 ADIO_Offset * len_list, int contig_access_count, ADIO_Offset
                                 ...
                                 ADIO_Offset * fd_start, ADIO_Offset * fd_end,
                                 MPI_Aint * buf_idx, int *error_code);
static void lchunk_alloc(ADIO_File fd, ADIO_Offset req_len,
		ADIO_Offset st_offset, ADIO_Offset offset_stride,
		int index);
static void add_log(ADIO_File fd, int myrank, uint8_t type, ADIO_Offset buf_offset,
                ADIO_Offset* offset_list, ADIO_Offset * len_list,
                int contig_access_count);
static int lchunk_is_new_pattern(ADIO_File fd, int index, ADIO_Offset len);
static int lchunk_is_full(ADIO_File fd, int index);
...
#endif
    ADIOI_Exch_and_write_pm(fd, (char *) buf, datatype, nprocs, myrank,
                         others_req, offset_list,
                         len_list, contig_access_count, min_st_offset,
                         fd_size, fd_start, fd_end, buf_idx, error_code);
...
static int exist_in_offset_list(ADIO_Offset offset, ADIO_Offset* offset_list,
		int contig_access_count)
{
    int ret = 0;
    
    for(int i = 0; i < contig_access_count; i++) {
        if(offset == offset_list[i])
	    ret = 1;
    }
...
/*Search the log to verify whether previous version has been
  recorded in the log*/
static int is_new_data_version(ADIO_File fd, ADIO_Offset* offset_list, 
        ADIO_Offset * len_list, int contig_access_count)
{
    int i, index, n_to_check = 10;
    ADIO_Offset cur_offset, offset_stride;
    lchunk_t *cur_lchunk = NULL;


    if(contig_access_count > 2)
        offset_stride = offset_list[1] - offset_list[0];
    else
        offset_stride = offset_list[0];

    n_to_check = fmin(n_to_check, contig_access_count);

    for(i = 0; i < n_to_check; i++) {
        index = (int)((offset_list[i] / (DISK_STRIPE_SIZE)) % N_IO_SERVER);
        cur_lchunk = fd->cur_lchunk[index];
        if(cur_lchunk) {
            if (cur_lchunk->req_len != len_list[i])
                return 1;

            if (cur_lchunk->offset_stride != offset_stride)
                return 1;

            if (!exist_in_offset_list(cur_lchunk->st_offset, offset_list, contig_access_count))
                return 1;
        } else
            break;
    }
...
static int lchunk_is_full(ADIO_File fd, int index)
{
    if(fd->cur_lchunk[index]->count >= N_ITEMS_PER_LCHUNK)
	return 1;
    else
	return 0;
}
...
static int lchunk_is_new_pattern(ADIO_File fd, int index, ADIO_Offset len)
{
    if(fd->cur_lchunk[index]->req_len != len)
	return 1;
    else
        return 0;
}
		...
		int index)
{
    ...
    new_lchunk = (lchunk_t *) ((uint64_t)fd->log_buf_addr + fd->log_buf_used); 
    fd->log_buf_used += LCHUNK_SIZE;
    ...
    if(fd->log_buf_used >= coll_bufsize) {
        printf("LOG FILE IS FULL!");
	exit(1);
    }
    
    memset(new_lchunk, 0, LCHUNK_SIZE);
    new_lchunk->id = fd->num_of_chunks;
    fd->num_of_chunks++;
    new_lchunk->count = 0;
    new_lchunk->disk_id = index;
    new_lchunk->req_len = req_len;
    new_lchunk->version = fd->version;
    new_lchunk->st_offset = st_offset;
    new_lchunk->offset_stride = offset_stride;
    new_lchunk->next = NULL;

    if(fd->cur_lchunk[index]) {
	cur_lchunk = fd->cur_lchunk[index];
        cur_lchunk->next = new_lchunk;
        // pmem_persist(cur_lchunk->next, sizeof(lchunk_t *));
    }
    
    fd->cur_lchunk[index] = new_lchunk;
        
    // pmem_persist(new_lchunk, LCHUNK_SIZE);
}
		...
		int contig_access_count)
{
    int log_index, i;
    lchunk_t *lchunk;
    ADIO_Offset cur_buf_offset = buf_offset;
    uint64_t offset_stride = 0;
    int server_index = 0;
    ...
    for (i = 0; i < contig_access_count; i++) {
        ...
        server_index = (int)((offset_list[i] / (DISK_STRIPE_SIZE)) % N_IO_SERVER);

        if(!fd->cur_lchunk[server_index] 
                || lchunk_is_full(fd, server_index)  
                || lchunk_is_new_pattern(fd, server_index, len_list[i])) {
            lchunk_alloc(fd, len_list[i], offset_list[i], offset_stride, server_index);
        }

        lchunk = fd->cur_lchunk[server_index];
        log_index = lchunk->count;
        log_item_t *item = &lchunk->items[log_index];
        item->type = type;
        item->ptr = cur_buf_offset;
        item->version_id = fd->version;
        item->offset = offset_list[i];
        cur_buf_offset += len_list[i];
        lchunk->count++;
    }
}
...
static void ADIOI_Exch_and_write_pm(ADIO_File fd, void *buf, MPI_Datatype
                                 ...
                                 * others_req, ADIO_Offset * offset_list,
                                 ...
                                 MPI_Aint * buf_idx, int *error_code)
{
    // printf("Call to exch and write by process %d\n", myrank);
    ADIO_Offset total_lens = 0;	
    int i; 

    *error_code = MPI_SUCCESS;

    for (i = 0; i < contig_access_count; i++)
        total_lens += len_list[i];

    if(is_new_data_version(fd, offset_list, len_list, contig_access_count))
        fd->version++;

    add_log(fd, myrank, NORMAL_LOG, fd->pm_cur_pos, offset_list, len_list, contig_access_count);
    ...
    fd->data_buf_used += total_lens;
    if(fd->data_buf_used >= coll_bufsize) {
        printf("DATA BUFFER IS FULL!\n");
        exit(1);
    }

    // pmem_memcpy_persist(fd->data_buf_addr+fd->pm_cur_pos, buf, total_lens);
    memcpy(fd->data_buf_addr + fd->pm_cur_pos, buf, total_lens);
    //pmem_memcpy(fd->data_buf+fd->pm_cur_pos, buf, total_lens, PMEM_F_MEM_NODRAIN);
    fd->pm_cur_pos += total_lens;
}
