	 ...
	 char **argv)
{
    ...
    if (my_rank == 0)
		report_parameters(nr_procs, nr_procs_used, sz_dataset);
    ...
    if (my_tile[1] == 0)
		...
		off_tile[1] = my_tile[1] * (sz_tile_x - overlap_x);

    if (my_tile[0] == 0)
		off_tile[0] = 0;
    else
		off_tile[0] = my_tile[0] * (sz_tile_y - overlap_y);
    ...
    char cbstring[20];
    sprintf(cbstring, "%d", sz_coll_buf*1024);
