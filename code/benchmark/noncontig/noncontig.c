...
" -outfile 	 filename used for writing parameters/metrics in CSV format\n"\
...
" -bufsize   size (KB) of memory buffer to use for access (multiple vector length)\n"\
...
void print_timing (int is_read, MPI_Offset data_size, MPI_Offset file_size, double t)
{
    ...
    if (!myrank) {
		...
		double min_bw_mb = (data_size/(t_max/loops))/MB;
		double max_bw_mb = (data_size/(t_min/loops))/MB;
		double acc_bw_mb = acc_bw / MB;

		printf(" %s bandwidth (min/max/acc [MB/s]) : %6.3f / %6.3f / %6.3f\n",
			bw, min_bw_mb, max_bw_mb, acc_bw_mb);

		if (is_read)
			printf(" file size: %llukB  size per process: %llukB\n",
				file_size/1024, data_size/1024);
			
		if (outfile != NULL) {
			fprintf(outfile, ",%f,%f,%f,%f,%f", min_bw_mb, max_bw_mb, acc_bw_mb, t_min, t_max);

			if (is_read) {
				fprintf(outfile, "\n");
			}
			/* Print error. Unhandled currently
			int bytes_written = fprintf(outfile, ",%f,%f,%f\n", min_bw_mb, max_bw_mb, acc_bw_mb);
		...
		MPI_Info finfo, int veclen, int elmtcount, int veccount)
{
    ...
    MPI_Aint data_size;
    int get_count;
    MPI_Offset err_cnt, acc_count, all_count;
    MPI_Offset i, c, l ,mpi_error;    
    double t0, t_total;
    ...
    if (!myrank) {
		...
		printf ("# vector count = %d - access count = %llu\n", veccount, acc_count);
		fflush (stdout);
		MPI_File_delete(filename, MPI_INFO_NULL);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    // printf("Writing output data to buffer...\n");
    for (i = 0; i < acc_count*veclen; i++) 
		buf[i] = i%veclen + myrank*veclen;
 ...
 write_exit:
    if (mpi_error != MPI_SUCCESS) {
       ...
       MPI_Error_string (mpi_error, string, &len);
       fprintf (stderr, "[%d] Error %llu in MPI_File_write\n%s\n",
                       myrank, mpi_error, string);
    } else {
       if (all_count != elmtcount*veccount*veclen) {
          printf("[%d] write-count is %llu should be %d\n",
 ...
 read_exit:
    if (mpi_error != MPI_SUCCESS) {
       ...
       MPI_Error_string (mpi_error, string, &len);
       fprintf (stderr, "[%d] Error %llu in MPI_File_read\n%s\n",
                       myrank, mpi_error, string);
    } else {
       if (all_count != veccount*elmtcount*veclen) {
          printf("[%d] read-count is %llu should be %d\n",
...
int main(int argc, char **argv)
{
    ...
    int elmtcount, stride, bytes_written;
    MPI_Aint d[3], bufsize;
    char *filename, *ionodes;
	char tmp[256], outfile_name[MAX_FNAME_LEN];
    MPI_Datatype elmttype, typevec, newtype, t[3];
    MPI_Info finfo;
    MPI_Offset displs, offset, fsize;
	...
	// -10 to account for appending "_{rank}"
    cmdline_get_string (&argc, argv, 1, "-outfile", outfile_name, MAX_FNAME_LEN);
    ...
    fsize = (MPI_Offset)veclen * (MPI_Offset)elmtcount * (MPI_Offset)veccount * sizeof(int);
    ...
    if (myrank == 0) {
		...
		printf("nprocs: %d\n", nprocs);
		printf("ionodes: %s\n", ionodes);
		printf("file size (MiB): %llu\n", fsize / (1024 * 1024));
		printf("buffer size: %ld \n", bufsize);
		printf("vector length: %d\n", veclen);
		printf("element count: %d\n", elmtcount);
		printf("vector count: %d\n", veccount);
		printf("striping factor: %d\n", sfact);
		printf("striping size: %d\n", ssize);
		printf("collective buffer size: %d\n", cbsize);
		printf("loops: %d\n", loops);
    ...
    if (myrank == 0 && strlen(outfile_name) != 0) {
		struct stat st;
		char *header_str = "nprocs,fsize,bufsize,veclen,elmtcount,veccount,"
						   "cbsize,loops,"
						   "w_min_bw,w_max_bw,w_acc_bw,w_time_min,w_time_max,"
						   "r_min_bw,r_max_bw,r_acc_bw,r_time_min,r_time_max\n";

		outfile = fopen(outfile_name, "a+");
		if (outfile == NULL) {
			printf("\n*# could not open outfile %s\n", outfile_name);
			MPI_Abort(MPI_COMM_WORLD, 1);
		}

		if (stat(outfile_name, &st) != 0) {
			printf("\n*# error stat'ing outfile %s\n", outfile_name);
			MPI_Abort(MPI_COMM_WORLD, 1);
		}
		/* Only print header if empty file */
		if (st.st_size == 0) {
			bytes_written = fprintf(outfile, "%s", header_str);
			if (bytes_written < strlen(header_str)) {
				printf("\n*# error writing to outfile %s\n", outfile_name);
				MPI_Abort(MPI_COMM_WORLD, 1);
			}
		}

		bytes_written = fprintf(outfile, "%d,%d,%ld,%d,%d,%d,%d,%d",
									nprocs, fsize, bufsize, veclen,
									elmtcount, veccount, cbsize, loops);
		if (bytes_written < 0) {
			printf("\n*# error writing to outfile %s\n", outfile_name);
			MPI_Abort(MPI_COMM_WORLD, 1);
		}
